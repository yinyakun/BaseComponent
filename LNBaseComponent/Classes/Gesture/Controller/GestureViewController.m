...
- (void)setupSameUI
{
    ...
    lockView.arrow = self.showPath;
    self.lockView = lockView;
    [self.view addSubview:lockView];

    PCLockLabel *msgLabel = [[PCLockLabel alloc] init];
    msgLabel.frame = CGRectMake(0, 0, kScreenW, 14);
    msgLabel.center = CGPointMake(kScreenW/2, CGRectGetMinY(lockView.frame) - 50);
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type connectCirclesLessThanNeedWithGesture:(NSString *)gesture
{
    ...
    if ([gestureOne length]) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"重置" style:UIBarButtonItemStylePlain target:self action:@selector(didClickRightItem)];
...
- (void)circleView:(PCCircleView *)view type:(CircleViewType)type didCompleteSetSecondGesture:(NSString *)gesture result:(BOOL)equal
{
    ...
    if (equal) {
        ...
        [PCCircleViewConst saveGesture:gesture Key:self.passwordSaveKey];
            //        [PCCircleViewConst saveGestureToNet:gesture];
            //        [PCCircleViewConst saveGesture:gesture Key:gestureUserIDSaveKey];
        if (self.makeGestureResult) {
            self.makeGestureResult(YES);
        }
