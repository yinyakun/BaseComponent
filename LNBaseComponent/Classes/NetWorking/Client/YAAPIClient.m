...
@property (nonatomic, strong) YAAPIBaseRequestDataModel *requestModel;
...
- (NSNumber *)callRequestWithRequestModel:(YAAPIBaseRequestDataModel *)requestModel{
    ...
    __block __weak NSURLSessionDataTask *task = [sessionManager
                                  ...
                                  completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error)
    {
        if (task.state == NSURLSessionTaskStateCanceling) {
            ...
            id _Nullable responseObject_c;
            if (requestModel.responseType == LNPSRefDataTypeXML) {
...
//                    responseObject_c = doc.rootElement.stringValue;
                
//                }
                responseObject_c = responseObject;
           ...
           [YAAPIResponseErrorHandler errorHandlerWithRequestDataModel:requestModel responseURL:response responseObject:responseObject_c error:error errorHandler:^(NSError *newError) {
                requestModel.responseBlock(responseObject_c, newError);
            }];
        }
    }];
    [task resume];
...
- (AFURLSessionManager *)sessionManager
{
    if (_sessionManager == nil) {
        ...
        if (self.requestModel.responseType == LNPSRefDataTypeXML) {
            AFHTTPResponseSerializer *responseSerializer =[AFHTTPResponseSerializer serializer];
            responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/javascript",@"text/xml", nil];
            _sessionManager.responseSerializer = responseSerializer;//[AFJSONResponseSerializer serializer];
        }else{
